...
/* eth0 mac address from ../../net/str8100/star_nic.c */
extern unsigned char tscavium_eth0_mac[6];
...
static void smsc95xx_init_mac_address(struct usbnet *dev)
{
	...
	/* set mac address to eth0 + 1 for TS products */
	for (i = 0; i < 3; i++){
		dev->net->dev_addr[i] = tscavium_eth0_mac[i];
	}

	x = tscavium_eth0_mac[3] << 16;
	x |= tscavium_eth0_mac[4] << 8;
	x |= tscavium_eth0_mac[5];
	x++;
	
	dev->net->dev_addr[3] = (x >> 16) & 0xff;
	dev->net->dev_addr[4] = (x >> 8) & 0xff;
	dev->net->dev_addr[5] = x & 0xff;
#else
